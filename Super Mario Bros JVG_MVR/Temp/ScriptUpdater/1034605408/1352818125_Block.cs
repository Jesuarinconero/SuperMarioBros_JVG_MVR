...
public class Block : MonoBehaviour
{
    ...
    void Break()
    {
        Mario mario = FindObjectOfType<Mario>(); // Encuentra a Mario en la escena
        if (mario != null && mario.currentState == Mario.State.Default)
        {
            return;
        }

        // Generar las piezas de ladrillo
        ...
        brickPiece = Instantiate(brickPiecePrefab, transform.position, Quaternion.Euler(0, 0, 90));
        brickPiece.GetComponent<Rigidbody2D>().linearVelocity = new Vector2(-3f, 12f);

        brickPiece = Instantiate(brickPiecePrefab, transform.position, Quaternion.Euler(0, 0, -90));
        brickPiece.GetComponent<Rigidbody2D>().linearVelocity = new Vector2(3f, -8f);

        brickPiece = Instantiate(brickPiecePrefab, transform.position, Quaternion.Euler(0, 0, 180));
        brickPiece.GetComponent<Rigidbody2D>().linearVelocity = new Vector2(-3f, -8f);
